...
Emits ``FUNDING_FORM_C`` with ``filing_type="Form C"``. The daily_run
...
def _form_c_evidence(
    *,
    ...
    Form C actually carries — no invention. Always non-empty."""
    lead = (
        f"Reg CF (Form C) filed {filed_on}" if filed_on
        else "Reg CF (Form C) filing"
    )
    extras: list[str] = []
    if offering_amount is not None:
        extras.append(f"${offering_amount:,.0f} maximum offering")
    if revenue is not None:
        extras.append(f"${revenue:,.0f} most-recent-year revenue")
    if current_employees is not None:
        extras.append(f"{current_employees} employees")
...
def _fetch_from_efts(
    ...
    for page in range(max_pages):
        ...
        for hit in hits:
            ...
            if not link:
                _log.info("edgar form-c: dropping %s (no filing link)", name)
                continue

            seen_names.add(key)
            state = (details or {}).get("state")
            city = (details or {}).get("city")
            offering_amount = (details or {}).get("offering_amount")
            revenue = (details or {}).get("revenue")
            candidates.append(
                LeadCandidate(
                    name=name,
