...
_MAX_RETRIES = 5
_BACKOFF_BASE_S = 1.5
# Per-request timeout. A grounded Gemini call legitimately takes 10-30s; 90s
...
def _with_retries(fn: Callable[[], Any], *, what: str) -> Any:
    ...
    for attempt in range(_MAX_RETRIES):
        try:
            ...
            time.sleep(wait)
        except (openai.APIConnectionError, httpx.TimeoutException, httpx.TransportError) as exc:
            # Timeout / dropped connection (openai.APITimeoutError subclasses
            # APIConnectionError; gemini's transport raises httpx errors).
            # Transient — retry with backoff rather than skipping the lead.
            last_exc = exc
            if attempt >= _MAX_RETRIES - 1:
                raise LLMError(f"{what}: timeout/connection error") from exc
            wait = _BACKOFF_BASE_S * (2**attempt)
            log.warning(
                "%s: timeout/connection error (attempt %d/%d), sleeping %.1fs",
                what, attempt + 1, _MAX_RETRIES, wait,
            )
            time.sleep(wait)
